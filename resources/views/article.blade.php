    ...
    <div class="relative py-16 bg-white overflow-hidden">
        <div class="relative px-4 sm:px-6 lg:px-8">
            <div class="text-lg max-w-prose mx-auto">
                <span class="block text-base text-center text-indigo-600 font-semibold tracking-wide uppercase">News</span>
                <h1>
            ...
            <div class="mt-6 prose prose-indigo prose-lg text-gray-500 mx-auto">
                ...
                @foreach($article->newsComments as $comment)
                    @if($comment->status == 1)
                            ...
                            <div class="d-flex" style="display: flex; align-items: center;">
                                <div class="comment-reply-container">
                                    ...
                                    <div class="comment-reply" style="display: none;">
                                        <form action="{{route('newsRepliesComment.create')}}" method="post">
                                            @csrf
                                ...
                                @auth
                                    @if(Auth::user()->name == $comment->author)
                                        <form action="{{route('newsComment.edit', $comment->id)}}" method="post" id="edit_form_{{$comment->id}}" style="display: none;">
                                            @csrf
                                            @method('PATCH')
                                        ...
                                        <form action="{{route('newsCommentOwner.delete', $comment->id)}}" method="post">
                                            @csrf
                                            @method('DELETE')
                    ...
                    @if(count($comment->replyNewsComments) > 0)
                        @php
                            $approvedRepliesCount = 0;
                            foreach ($comment->replyNewsComments as $reply) {
                                if ($reply->status == 1) {
                                    $approvedRepliesCount++;
                                }
                            }
                        @endphp
                        @if($comment->replyNewsComments()->count() > 0 && $approvedRepliesCount > 0 && $comment->status == 1 )
                            <button class="toggle-replies btn btn-primary pull-left mr-3">
                                <i class="fas fa-caret-down arrow"></i>
                                {{$approvedRepliesCount}}
                                <span class="reply-text">@if($comment->replyNewsComments()->count() == 1)
                                        <strong>reply</strong>
                                    @else
                ...
                <div class="comment-area mt-24">
                    ...
                    <form action="{{route('newsComment.create')}}" method="POST" >
                        @csrf
